...
1. `git clone https://github.com/blackdeltatechnologies/Traffic-Signal-Violation-Detection-System.git`
2. Download the required files that are not included in the repository:
   - YOLOv3 weights file: Download from [here](https://pjreddie.com/media/files/yolov3.weights) and place in the root directory
   - Sample videos: Create a `Resources` folder with `input` and `output` subfolders
   - Example videos can be downloaded from [this folder](https://drive.google.com/drive/folders/your-folder-id) (optional)
3. Change the directories in "Project-GUI.py" & "object_detection.py" if needed
4. Install required python dependencies into your python virtual environment.
5. `python Project-GUI.py`

Note: Large binary files (videos, weights file, etc.) are not included in the repository to keep it lightweight. Please download them separately as mentioned above.
